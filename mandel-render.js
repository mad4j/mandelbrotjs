...
smooth=1;else
smooth=0;let smoothMandel=new Uint8Array(e.data.smoothMandel);let segmentHeight=blockSize==1?300:150;let lblockSize=blockSize==1?1:blockSize/2;for(let y=0;y<segmentHeight;y+=lblockSize){
    // Check if job was cancelled
    if(!isRunning || currentJobId !== jobId) {
        return;
    }
    for(let x=0;x<arrayWidth;x+=lblockSize){out=mandel[x+y*arrayWidth];if(out==255){r=0;g=0;b=0;}
