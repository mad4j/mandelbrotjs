...
{const blockSize=e.data.blockSize;const arrayWidth=e.data.arrayWidth;let pixels=new Uint8Array(e.data.canvasBuffer);const workerID=e.data.workerID;let mandel=new Uint8Array(e.data.mandel);let colours=e.data.colours;let pixelPos=0;const larrayWidth=arrayWidth;let out=0;let packedColour=0;let packedColour1=0;let r=0;let g=0;let b=0;let r1=0;let g1=0;let b1=0;if((e.data.smooth==1)&&(blockSize==1))
smooth=1;else
smooth=0;let smoothMandel=new Uint8Array(e.data.smoothMandel);let segmentHeight=Math.floor(pixels.length/(arrayWidth*4));let lblockSize=blockSize==1?1:blockSize/2;for(let y=0;y<segmentHeight;y+=lblockSize){for(let x=0;x<arrayWidth;x+=lblockSize){out=mandel[x+y*arrayWidth];if(out==255){r=0;g=0;b=0;}
