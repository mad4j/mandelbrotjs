...
if(zooming){zoomTime=performance.now();zoom=Math.floor(zoom);if(autotuneIterations){iterations=calculateAdaptiveIterations(zoom);iterSlider.value=iterations;itersInput.value=iterations;}screenX=Math.round(-xnorm*zoom+canvasWidth/2);screenY=Math.round(-ynorm*zoom+canvasHeight/2);eventOccurred=1;startRender(1,1);}}
