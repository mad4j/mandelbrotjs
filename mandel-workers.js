...
// Larger chunks reduce JS-WASM boundary overhead and leverage WASM's superior performance
...
function drawMandel()
{const iter_max=iterations;const lcanvasWidth=canvasWidth;const lcanvasHeight=canvasHeight;const lscreenX=screenX;const lscreenY=screenY;const lzoom=zoom;
    
    // Recalculate optimal chunk height based on current zoom and iterations for WASM performance
    const currentChunkHeight = calculateOptimalChunkHeight(canvasHeight, workers, zoom, iterations);
    
    if((rotating)&&(performance.now()<rotationFrameStart+15)){requestAnimationFrame(drawMandel);return 1;}
...
needRedraw=1;if(needRedraw){for(i=0;i<workers;i++){
    // Use dynamic chunk height and ensure we don't exceed canvas bounds
    startLine = Math.min(currentChunkHeight * i, canvasHeight - 1);
    const actualSegmentHeight = Math.min(currentChunkHeight, canvasHeight - startLine);
    
    if((needToRun[i]==1)&&(!eventOccurred)&&(actualSegmentHeight > 0)){if(renderWorkerRunning[i]){continue;}
