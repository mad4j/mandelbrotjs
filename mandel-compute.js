...
for(let y=yStart;y<segmentHeight;y+=lblockSize){
    ...
    let ynorm=(y+startLine-screenY)/zoom;for(let x=xStart;x<canvasWidth;x+=lblockSize){let xnorm=(x-screenX)/zoom;let iteration=firstIteration;if((xnorm>-0.5)&&(xnorm<0.25)&&(ynorm>-0.5)&&(ynorm<0.5))
...
lineCounter++;if((blockSize==1)&&(lineCounter==20)){
    // Check if job was cancelled before sending progress
    if(!isRunning || currentJobId !== jobId) {
        return;
    }
    self.postMessage({lineCount:y,workerID:workerID});lineCounter=0;}}
// Check if job was cancelled before sending final result
if(!isRunning || currentJobId !== jobId) {
    return;
}
